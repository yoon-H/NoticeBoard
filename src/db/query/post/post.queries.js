const SQL_POST_QUERIES = {
  ...
  GET_POST:
    "SELECT p.id AS id, p.title AS title, u.name AS author, p.content AS content, p.create_dt AS time FROM posts AS p LEFT JOIN users AS u ON p.author = u.id WHERE p.id = ?;",
  EDIT_POST:
